...
try {
		/*if(isset($_POST['id']))
		{
		 	...
		 	{ 
		    	...
		    	// execute the query
		    	$stmt->execute();
		    	$sql = "update relation_information_prestataire set valide = 0 where n_sequence ='".$id[$i]."'";
		    	// Prepare statement
		    	$stmt = $conn->prepare($sql);
		    	// execute the query
		    	$stmt->execute();
		...
		*/
		...
		$sql = "update `prestataire` SET `nbr_signalement_payee` = `nbr_signalement_payee`+".sizeof($listID)." WHERE num_serie='".$num_serie."'";
    	// Prepare statement
    	$stmt = $conn->prepare($sql);
    	// execute the query
    	$stmt->execute();
