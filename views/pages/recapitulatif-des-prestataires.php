        ...
        <div class="card">
          ...
          <div class="card-content collapse show">
            ...
            <div class="card-body card-dashboard">
              ...
              <input type="hidden" id="get_list_id" value="">
            </div>
          </div>
        </div>
      </div>
                       ...
                       <td>
                         <div class="form-group">
                           <div class="position-relative has-icon-left">
                             ...
                             <select class="select2 form-control" multiple="multiple" name="listInfo" size="6" id="listInfo" onchange="Set_Number();calcul_montant('nombre2','montant2','total2')">
                              </select>
                              <button type="button" class="btn btn-primary" onclick="Save_Invalid_Information()">Enregistrer les information invalides</button>
                           </div>
                         </div>
                       </td>
                       <td></td>
...
function Set_Number()
{
  ...
  if(nb>=$('#listInfo').val().length)
  $('#nombre2').val(nb-$('#listInfo').val().length);
}
...
{ 
  ...
  $.ajax({
        type:"POST",
        data:{num_serie:num_serie,date:$('input[name=begin_date]').val()+"-01"},
...
function Save_Invalid_Information()
{
  $.ajax({
        data:{ 
               
               id:$('#listInfo').val()+"",
               num_serie:$('input[name=get_data_id]').val()
               
           } ,
        type: "post",
        url: "../Actions/Information_Actions/Invalid_Information_Action.php",
        dataType:'JSON',
        success: function(data){
          if(data['success']=='success')
          {
           
           swal("Success","Modification est enregistrée avec success","success");
...
function get_all_data()
{
  ...
  $(document).ready(function(){
    if(parseFloat(new Date().getMonth() + 1) < 10 )
      $('input[name=begin_date]').val(new Date().getFullYear()+'-0'+(new Date().getMonth() + 1));
    else 
