...
if($_POST['filter']!="")
  ...
  $filter = "";
try{
    $stmt = $conn->prepare("select *,P.abonnement as Ab,SUM( + (FC.`nbr_Signalement` * P.honoraire) )as m , SUM(nbr_Signalement) as nbr_Signalement_per_month FROM `facture_client` as FC, prestataire as P where P.num_serie = FC.num_serie GROUP BY P.num_serie
 HAVING raison_sociale like '%".$filter."%' or date_facture like '%".$filter."%'");
      $stmt->execute();
      ...
      while ($row = $stmt->fetch()) 
      {
       echo '<tr>
                      ...
                      <td>'.$row['nbr_Signalement_per_month'].'</td>
                      <td>'.(float)($row['m']+$row['Ab']).'</td>
                      ...
                      echo '<td>
                        <div class="dropdown">
                          ...
                          <div class="dropdown-menu" aria-labelledby="dropdownMenuButton2" x-placement="right-start">
                            <a class="dropdown-item" onclick="get_all_data()" href="../Actions/Bill_Actions/Bill_Prestataire_PDF2.php?num='.$row['num_serie'].',&date='.$row['date_facture'].'"  target="_blank" >Imprimer facture</a>
