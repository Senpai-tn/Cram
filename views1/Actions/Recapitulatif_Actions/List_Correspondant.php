...
try{
	    ...
	    $stmtV = $conn->prepare("select RIC.paye,GROUP_CONCAT(RIC.id) as N, C.nom,C.code,C.prix,SUM(RIC.valide) as nbr_valide, count(*) as total  FROM relation_information_correspondant as RIC,correspondant as C WHERE RIC.code = C.code and (MONTH(RIC.created_at) = MONTH('".$begin_date->format('Y-m-d')."') OR MONTH(RIC.created_at)=MONTH('".$end_date->format('Y-m-d')."')) and (YEAR(RIC.created_at)=YEAR('".$begin_date->format('Y-m-d')."') Or YEAR(RIC.created_at)= YEAR('".$end_date->format('Y-m-d')."')) GROUP BY RIC.code");
	    $stmtV->execute();
		...
		while ($row = $stmtV->fetch()) 
	    {
          ...
          <td>';
              ...
              <div class="dropdown-menu" aria-labelledby="dropdownMenuButton2" x-placement="right-start">
                <a class="dropdown-item" href="#" data-toggle="modal" data-target="#modal-edit" onclick="select_prestataire();get_correspondant(\''.$row['code'].'\',\''.$row['nom'].'\');Show_UI(\''.$row['prix'].'\',\''.(int)($row['nbr_valide']).'\',\''.$row['N'].'\');">Facturer</a>
              </div>
            </div>';
